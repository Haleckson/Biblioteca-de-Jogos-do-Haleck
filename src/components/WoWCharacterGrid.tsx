...
export const WoWCharacterGrid: React.FC<WoWCharacterGridProps> = ({
  ...
  const alphabeticalCharacters = useMemo(() => {
    ...
    const filtered = characters.filter((c) => {
      if (currentVersion !== "all") {
        ...
        else if (raw.includes("mop") || raw.includes("pandaria")) key = "mop";
        else key = "retail";
  ...
  if (isLoading) {
    return (
      <div id="wow-grid-loading" className="py-10 flex flex-col items-center justify-center text-center space-y-3">
        ...
        <p className="text-xs font-medium text-zinc-300">Loading Blizzard characters...</p>
        <p className="text-[11px] text-zinc-500">Syncing Battle.net account roster and character data</p>
      </div>
    );
  }
  ...
  if (characters.length === 0) {
    return (
      <div className="p-6 text-center bg-zinc-950/40 rounded-xl border border-dashed border-zinc-800 space-y-2">
        ...
        <p className="text-xs font-medium text-zinc-400">No World of Warcraft characters loaded yet.</p>
        <p className="text-[11px] text-zinc-500">Click "Sync Battle.net" to load your character roster.</p>
      </div>
    );
  }
  ...
  return (
    <div id="wow-character-selector-container" className={`space-y-3 ${className}`}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-zinc-400 truncate">
                        {cRace.name} • <span style={{ color: cClass.color }}>{cClass.name}</span> • {activeChar.realm}
                      </p>
                    </div>
                  </div>
                );
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              title="Click to view characters list in alphabetical order"
            >
              <ArrowDownAZ size={15} />
              <span>Characters (A-Z)</span>
            ...
            <button
              ...
              className="p-2 rounded-xl text-zinc-400 hover:text-white bg-zinc-800/80 hover:bg-zinc-700 border border-zinc-700 transition-colors cursor-pointer"
              title={viewMode === "list_az" ? "Switch to Grid View" : "Switch to A-Z List View"}
            >
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search characters by name (A-Z), realm, or class..."
              ...
              {/* Version Filters: 5 Authentic WoW Versions */}
              {showVersionTabs && (
                <div className="flex items-center gap-1 overflow-x-auto pb-1 sm:pb-0 scrollbar-none text-[11px]">
                  ...
                  <button
                    type="button"
                    onClick={() => handleVersionChange("mop")}
                    className={`px-2.5 py-1 rounded-lg font-bold transition-all whitespace-nowrap cursor-pointer ${
                      currentVersion === "mop"
            ...
            <div className="flex items-center justify-between text-[11px] text-zinc-400 px-1 font-mono">
              <span className="flex items-center gap-1">
                ...
                <span>Sorted A to Z ({alphabeticalCharacters.length} displayed)</span>
              </span>
            ...
            <div className="max-h-72 overflow-y-auto pr-1 space-y-1.5 custom-scrollbar divide-y divide-zinc-800/40">
              {alphabeticalCharacters.length === 0 ? (
                <div className="py-6 text-center bg-zinc-950/40 rounded-xl border border-dashed border-zinc-800">
                  <p className="text-xs text-zinc-400">No characters found matching current filters.</p>
                </div>
              ) : (
      ...
      {/* FULL EXPANDED GRID VIEW */}
      {viewMode === "grid" && (
        <div className="space-y-6 pt-2">
          {["retail", "forever", "classic", "tbc", "mop"].map((vKey) => {
                ...
                <div className="flex items-center justify-between flex-wrap gap-2 pb-2 border-b border-zinc-800/80">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${versionMeta.badgeBg} ${versionMeta.borderClass} ${versionMeta.textClass}`}>
                      Level Cap: {versionMeta.levelCap}
                    </span>
                  </div>
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="flex-1 min-w-0 pr-10">
                            ...
                            <div className="flex items-center gap-1.5 mt-1 text-[11px]">
                              ...
                              <span className="text-zinc-600">•</span>
                              <span className="text-zinc-300 truncate">{raceInfo.name}</span>
                            </div>
                          </div>
                        </div>
